...
This repository contains a [CQL](https://cql.hl7.org/) engine that uses a [a fork](https://github.com/umg-minai/clinical_quality_language) of the [CQL reference implementation](https://github.com/cqframework/clinical_quality_language) and works with clinical data in databases which conform to the [OMOP Common Data Model](https://www.ohdsi.org/data-standardization/).
...
Since this project is currently using [a fork](https://github.com/umg-minai/clinical_quality_language) of the [CQL reference implementation](https://github.com/cqframework/clinical_quality_language).
...
As a result, this project has to be built by first building the CQL engine. All required steps could be performed like this:
...
Successful compilation produces a standalone application in a file like `target/cql-on-omop-1.1.2-SNAPSHOT.jar`.
...
```bash
CQL_ON_OMOP_DATABASE_PASSWORD=$(GET-PASSWORD) java -jar REPOSITORY-DIRECTORY/target/cql-on-omop-1.1.2-SNAPSHOT.jar \
